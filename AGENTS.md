...
agents-radar is a daily digest generator for the AI open-source ecosystem. A GitHub Actions cron job runs at `37 22 * * *` UTC = 06:37 CST (deliberately off the hour — GitHub drops queued `:00` scheduled runs; see `daily-digest.yml`) and produces bilingual (Chinese + English) reports, published as GitHub Issues and committed Markdown files.
...
- **CLI_REPOS** (7, per `config.yml`): claude-code, codex, gemini-cli, copilot-cli, opencode, pi, qwen-code
- **Discussions** (`discussions: true` in `config.yml`): codex, pi
- **OPENCLAW** + **OPENCLAW_PEERS** (4): openclaw/openclaw + hermes-agent, ironclaw, qwenpaw, zeroclaw
